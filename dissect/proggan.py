...
def from_pth_file2(filename):
    ...
    if 'state_dict' in state_dict:
        state_dict = state_dict['state_dict']
    # Convert old version of parameter names
    if 'scaleLayers.0.0.module.weight' in state_dict:
        state_dict = state_dict_from_old_pt_dict2(state_dict)
    sizes = sizes_from_state_dict(state_dict)
    print(sizes)
    result = ProgressiveGenerator(sizes=sizes)
    result.load_state_dict(state_dict, strict=False)
...
def state_dict_from_old_pt_dict2(params):
    '''
    Conversion from the old pytorch model layer names.
    '''
    result = {}
